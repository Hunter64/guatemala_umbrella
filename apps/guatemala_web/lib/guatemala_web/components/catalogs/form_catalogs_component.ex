defmodule GuatemalaWeb.FormCatalogsComponent do
  @moduledoc """
  Form Catalogs Component
  """
  ...
  def update(attrs, socket) do
    ...
    catalog = if attrs.edit, do: attrs.id |> get_catalog_by_id(), else: nil
  ...
  def render(assigns) do
    ~H"""
      <div id="form_catalogs" class="bg-amber-100 dark:bg-slate-800 mt-16 ml-1 w-96 h-hoch-93 rounded border float-left">
        ...
        <div class="inline-block bg-amber-700 dark:bg-slate-800 text-white px-6 py-4 w-full">
          <div class="inline-flex top-0 right-0 bg-amber-700 dark:bg-slate-800 text-white mt-2">
            <button phx-click="close" phx-target="#catalogs" class="ml-80 -mt-4 text-white font-bold rounded shadow">
        ...
        <div class="h-hoch-80 px-8 w-full py-6 -mt-8 relative">
          <form id="form1" phx-submit={"submit_form"} phx-change={"update_form"} phx-target="#catalogs">
            <input type="hidden" name="action" value={if @new, do: 1, else: 2}>
            <input type="hidden" name="id" value={if @edit, do: @catalog.id}>
