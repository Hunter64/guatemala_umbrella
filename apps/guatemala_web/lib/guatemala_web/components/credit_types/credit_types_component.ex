defmodule GuatemalaWeb.CreditTypeComponent do
  @moduledoc """
    Componente CreditTypes
  """
  use Phoenix.LiveComponent
  use Phoenix.HTML

  alias Guatemala.CreditTypes, as: CreditTypes
  ...
  def render(assigns) do
    ~H"""
      <div id="credit_types" class="inline-flex w-full">
          ...
          <div class="h-hoch-93 w-80 mt-16 ml-16 block float-left bg-amber-100 dark:bg-slate-800">
            <div class="w-full py-2 bg-amber-700 dark:bg-slate-800">
              <p class="ml-2 font-bold text-lg text-white">Tipos de Crédito</p>
            </div>


            <div class="w-1/2 px-2 mt-2">
              <button phx-click="new_credit_type" phx-target="#credit_types" class="py-2 bg-green-500 dark:bg-slate-800 hover:bg-green-400 text-white items-center inline-flex font-bold rounded text-sm w-full">
            ...
            <div class="w-full mt-2 px-2">
              <label class="text-sm text-black dark:text-white">Tipos de Crédito: <%= @credit_types |> length %> </label>
            </div>
          </div>

          <div class="w-full">
            <%= @credit_types |> Enum.count %>
            <%= if @new, do: live_component(GuatemalaWeb.FormCreditTypesComponent, id: "credit_type", new: true, edit: false, form_valid: @form_valid, form: @form) %>
            <%= if @edit, do: live_component(GuatemalaWeb.FormCreditTypesComponent, id: @credit_type_id, new: false, edit: true, form_valid: @form_valid, form: @form) %>
  ...
  def handle_event("new_credit_type", _params, socket) do
    IO.puts("HERE ------------> NEW ")
    {:noreply, assign(
      socket,
      new: true,
  ...
  def handle_event("open_edit_credit_type", params, socket) do
    IO.puts("HERE ------------> EDIT ")
    {:noreply, assign(
      socket,
      new: false,
      edit: true,
      form: params["id"] |> String.to_integer |> get_credit_type_by_id() |> IO.inspect(label: " ---------------> FORM "),
      form_valid: init_fill_form(),
      credit_type_id: params["id"] |> String.to_integer
      )}
  end
  ...
  def get_credit_type_by_id(id) do
    CreditTypes.get_credit_type!(id)
  end
  ...
  def run_action(@new_action, params, socket) do
    ...
    validate_duplicated(credit_type_pre, socket)
      |> case do
        true ->
          CreditTypes.create_credit_type(credit_type_pre)
            |> case do
              {:ok, credit_type}
                ->
                  update_socket_result(socket, "Éxito", "El Tipo de Catálogo " <> credit_type.name <> " creado correctamente", @success_message, @new_action, 0, "succesfully")
  ...
  def run_action(@edit_action, params, socket) do
    credit_type = params["id"] |> String.to_integer |> get_credit_type_by_id()
    credit_type_pre =
      %{
      ...
      validate_duplicated(credit_type_pre, socket, credit_type.id)
      |> case do
        true ->
          # change = Guatemala.CreditTypes.CreditType.changeset(%Guatemala.CreditTypes.CreditType{}, credit_type_pre)

          are_changes(credit_type, credit_type_pre)
            |> case do
              true ->
                CreditTypes.update_credit_type(credit_type, credit_type_pre)
                |> case do
                  {:ok, credit_type}
                    -> update_socket_result(socket, "Éxito", "Tipo de Catálogo " <> credit_type.name <> " editado correctamente", @success_message, @edit_action, credit_type.id, "succesfully")
                  {:error, %Ecto.Changeset{} = changeset}
                    -> update_socket_result(socket, "Error", "Error al intentar actualizar el Tipo de Catálogo " <> EctoUtil.get_errors(changeset), @error_message, @edit_action, credit_type.id, "error")
                end

              false ->
                update_socket_result(socket, "Advertencia", "No existen cambios por aplicar, favor de revisar", @warning_message, @edit_action, credit_type.id, "warning")
            end


        false ->
          update_socket_result(socket, "Advertencia", "El registro ya existe en la base de datos, favor de revisar.", @warning_message, @edit_action, credit_type.id, "warning")
      end
  end

  def update_socket_result(socket, header, msg, status_message, action, credit_type_id, notification_type) do
    status_message
      |> case do
        @success_message ->
    ...
    {:noreply, assign(
      socket,
      credit_types: get_credit_types(),
      ##credit_types_non_filtered: get_credit_types(),
  ...
  def validate_duplicated(credit_type_pre, socket) do
    socket.assigns.credit_types
      |> Enum.map(fn x -> x.name |> String.downcase() end)
      |> compare_names(credit_type_pre.name |> String.downcase)
  end
  def validate_duplicated(credit_type_pre, socket, credit_type_id) do
    socket.assigns.credit_types
      |> Enum.reject(fn credit_type -> credit_type.id == credit_type_id end)
      |> Enum.map(fn x -> x.name |> String.downcase() end)
      |> compare_names(credit_type_pre.name |> String.downcase)
  end
  ...
  def are_changes(credit_type, credit_type_pre) do
    !(credit_type.name == credit_type_pre.name && credit_type.description == credit_type_pre.description)
  end
  ...
  def get_credit_types() do
    CreditTypes.list_credit_types
  end
