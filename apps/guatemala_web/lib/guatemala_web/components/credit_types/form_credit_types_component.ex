defmodule GuatemalaWeb.FormCreditTypesComponent do
  @moduledoc """
  Form CreditTypes Component
  """
  ...
  def update(attrs, socket) do
    credit_type = if attrs.edit, do: attrs.id |> get_credit_type_by_id(), else: nil
  ...
  def render(assigns) do
    ~H"""
      <div id="form_credit_types" class="bg-amber-100 dark:bg-slate-800 mt-16 ml-1 w-96 h-hoch-93 rounded border float-left">
        ...
        <div class="inline-block bg-amber-700 dark:bg-slate-800 text-white px-6 py-4 w-full">
          <div class="inline-flex top-0 right-0 bg-amber-700 dark:bg-slate-800 text-white mt-2">
            <button phx-click="close" phx-target="#credit_types" class="ml-80 -mt-4 text-white font-bold rounded shadow">
          ...
          <label class="text-2xl font-normal text-white block"> <%= if @new, do: "Crear Nuevo Tipo de Catálogo", else: "Editar Tipo de Catálogo" %></label>
        </div>

        <div class="h-hoch-80 px-8 w-full py-6 -mt-8 relative">
          <form id="form1" phx-submit={"submit_form"} phx-change={"update_form"} phx-target="#credit_types">
            <input type="hidden" name="action" value={if @new, do: 1, else: 2}>
            <input type="hidden" name="id" value={if @edit, do: @credit_type.id}>
